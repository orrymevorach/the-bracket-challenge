...
async function run() {
  ...
  const allBrackets = await airtableBase('User Brackets').select().all();

  for (let bracket of allBrackets) {
    const bracketId = bracket.getId();
    // const bracketId = 'recaNSPszvGcl5xFi';
    ...
    try {
      console.log('Success! Updated:', bracketId);
      const response = await airtableBase('User Brackets').update([
        {
          id: bracketId,
          fields: { Selections: json },
        },
      ]);
    } catch (error) {
