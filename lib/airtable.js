...
export const getBracketPicks = async ({ recId }) => {
  try {
    const { data } = await client.query({
      query: GET_BRACKET_PICKS,
      fetchPolicy: 'no-cache',
      variables: {
        recId,
      },
...
export const getUserLeagueData = async ({ uid }) => {
  const { user, winnersData } = await fetch('/api/airtable/hot-patch', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    },
  ...
  const res = await fetch('/api/airtable/get-user-league-data', {
    ...
    body: JSON.stringify({ user, winnersData }),
  }).then(res => res.json());
