...
export async function getQuestionsBySport({ sport }) {
  const { records: questions } = await getRecordsByFieldValue({
    tableId: 'Questions',
    formulaArray: [
      {
        fieldName: 'Sport',
...
export async function getSession({ recIds, snowboarders }) {
  const mapRecIdToSnowboarder = snowboarders.reduce((acc, snowboarder) => {
    acc[snowboarder.id] = snowboarder;
    return acc;
  }, {});
  const sessions = await Promise.all(
    recIds.map(async recId => {
      const { record: session } = await getRecordById({
        tableId: 'Sessions',
        recordId: recId,
      });
      const options = session.optionsRef.map(snowboarderRef => {
        const snowboarder = mapRecIdToSnowboarder[snowboarderRef];
...
export async function getSessionsBySport({ sport }) {
  const { records: sessions } = await getRecordsByFieldValue({
    tableId: 'Sessions',
    formulaArray: [
      {
        fieldName: 'Sport',
...
export async function getSport({ sport }) {
  const { records: sports } = await getRecordsByFieldValue({
    tableId: 'Sports',
    formulaArray: [
      {
        fieldName: 'Name',
