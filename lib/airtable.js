...
import {
  ...
  EDIT_LEAGUE_NAME,
  GET_LEAGUE_IDS,
  GET_SNOWBOARDERS_DEVELOPMENT_MODE,
} from 'graphql/queries';
import { router } from 'next/router';

const getIsDevelopmentMode = asPath => asPath.includes('developmentMode=true');

export const getSnowboarders = async () => {
  const isProduction = !getIsDevelopmentMode(router.state.asPath);
  try {
    ...
    const snowboarders = isProduction
      ? data.snowboarders
      : data.snowboardersDevelopmentMode;
...
export const getWinners = async () => {
  ...
  const name = isProduction
    ? 'Master Winners Bracket'
    : 'Master Winners Bracket Development Mode';
  try {
    const { data } = await client.query({
      query: GET_WINNERS,
      variables: {
