      ...
      const matchupsData = await Promise.all(
        matchups.map(async matchupId => {
          ...
          const actualWinner = matchup.actualWinner
            ? snowboardersAsMap[matchup.actualWinner]
            : '';
          return {
            ...matchup,
            team1,
...
export async function getQuestions({ recIds }) {
  const questions = await Promise.all(
    recIds.map(async recId => {
      ...
      const options = await Promise.all(
        question.optionsRef.map(async snowboarderRef => {
          const { record: snowboarder } = await getRecordById({
            tableId: 'Snowboarders',
            recordId: snowboarderRef,
          });
          return {
